...
const menuData = {
    rotis: [
        ...
        { id: 2, name: "Butter Roti", price: 15, image: "images/menu/butter-roti.jpg" },
        { id: 3, name: "Naan", price: 20, image: "images/menu/naan.jpg" },
        { id: 4, name: "Butter Naan", price: 25, image: "images/menu/butter-naan.jpg" },
        { id: 5, name: "Garlic Naan", price: 30, image: "images/menu/garlic-naan.jpg" }
    ],
    curries: [
        { id: 6, name: "Punjabi Chicken", price: 220, image: "images/menu/pc.jpg" },
        { id: 7, name: "MahaRaja Chicken", price: 260, image: "images/menu/mc.jpg" },
        { id: 8, name: "Rombha Chicken", price: 250, image: "images/menu/rc.jpg" }
    ],
    biryanis: [
        { id: 9, name: "Veg Biryani", price: 120, image: "images/menu/veg-biryani.jpg" },
        { id: 10, name: "Chicken Biryani", price: 180, image: "images/menu/chicken-biryani.jpg" },
        { id: 11, name: "Mutton Biryani", price: 220, image: "images/menu/mutton-biryani.jpg" }
    ],
    friedrice: [
        { id: 12, name: "Veg Fried Rice", price: 100, image: "images/menu/veg-fried-rice.jpg" },
        { id: 13, name: "Egg Fried Rice", price: 130, image: "images/menu/egg-fried-rice.jpg" },
        { id: 14, name: "Chicken Fried Rice", price: 150, image: "images/menu/chicken-fried-rice.jpg" }
    ],
    drinks: [
...
function initTablesView() {
    ...
    // Handle close button
    document.querySelector('.close-btn')?.addEventListener('click', closeTableDetails);
...
function updateTableStatus() {
    ...
    tableCards.forEach(card => {
        ...
        if (state.orders[tableNumber] && state.orders[tableNumber].items.length > 0) {
    ...
    Object.keys(state.orders).forEach(tableNumber => {
        ...
        if (order.items.length > 0) {
            ...
            billElement.innerHTML = `
                ...
                <button class="btn clear-bill" data-table="${tableNumber}">Clear Bill</button>
            `;
            
            billElement.querySelector('.clear-bill').addEventListener('click', function() {
...
function initReportsView() {
    ...
    // Handle date change
    document.getElementById('reportDate')?.addEventListener('change', loadDailyReports);
    
    // Handle download button
    document.querySelector('.download-report')?.addEventListener('click', downloadDailyReport);
}
...
function downloadDailyReport() {
    ...
    Object.keys(state.bills).forEach(tableNumber => {
        state.bills[tableNumber].forEach(bill => {
            const billDate = new Date(bill.date).toISOString().split('T')[0];
            if (billDate === date) {
                reportContent += `Table ${tableNumber} - ${new Date(bill.date).toLocaleTimeString()}\n`;
                
                bill.items.forEach(item => {
                    reportContent += `${item.name} (${item.quantity}) - ₹${item.price * item.quantity}\n`;
                });
...
function initSettingsView() {
    ...
    document.querySelector('.add-item')?.addEventListener('click', addNewMenuItem);
}
